...
def add_small_noise(signal, noise_level=0.003):
    noise = np.random.randn(len(signal)).astype(np.float32)
    return signal + noise_level * noise


def apply_gain(signal, low=0.9, high=1.1):
    scale = np.random.uniform(low, high)
    return signal * scale


def shift_waveform(signal, max_shift_ratio=0.03):
...
def maybe_augment(signal, lang_name):
    # do nothing for languages outside the selected set
    if lang_name not in AUGMENT_LANGS:
        return signal

    # keep augmentation probabilistic so not every selected sample changes
    if random.random() < 0.35:
        signal = add_small_noise(signal, noise_level=np.random.uniform(0.0015, 0.005))

    if random.random() < 0.30:
        signal = apply_gain(signal, low=0.92, high=1.08)

    if random.random() < 0.20:
        signal = shift_waveform(signal, max_shift_ratio=0.02)
...
def prepare_train_batch(batch):
    ...
    for audio_obj, lang_id in zip(batch[AUDIO_FIELD], lang_ids):
        ...
        wav = maybe_augment(wav, lang_name)
